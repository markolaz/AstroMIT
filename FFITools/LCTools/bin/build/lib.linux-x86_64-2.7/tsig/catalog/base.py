...
class Catalog(ConfigurableObject):
    ...
    def __init__(self):
        ...
        self.teff = []
        self.lightcurve = []
        self.pmra = [] # rate of change for ra in mas/year
        self.pmdec = [] # rate of change for dec mas/year
    ...
    def get_cache_dirname():
        return os.path.join(os.environ['HOME'], '.cache/tsig')

    @staticmethod
    def create_cache_dir(dirname=None):
        if dirname is None:
            dirname = Catalog.get_cache_dirname()
        try:
            os.makedirs(dirname)
        except OSError:
            pass
    ...
    def create_query_name(catalog, ra, dec,
        ...
        base = "{catalog}_ra{ra:+09.4f}dec{dec:+09.4f}".format(
    ...
    def add_lightcurves(self, faintest_star=None, stars_with_random_curves=0.0):
        ...
        self.lightcurve = np.array([c] * nstar)
        ...
        if stars_with_random_curves:
            ...
            if faintest_star is None:
                faintest_star = np.max(self.tmag) + 1
            logger.debug('magnitude dimmest=%s threshold=%s' %
                         (np.max(self.tmag), faintest_star))
            # do only stars that are bright enough
            bright_stars = (self.tmag <= faintest_star).nonzero()[0]
    ...
    def stars_snapshot(self, bjd=None, epoch=None, exptime=0.5 / 24.0,
        ...
        if bjd is not None:
            ...
            if epoch is None:
                epoch = 2000.0 # FIXME: what should default epoch be?
        ...
        tmag = self.tmag
        if len(tmag):
            if len(self.lightcurve) == len(tmag):
                moment = np.array([lc.integrated(bjd, exptime)
                                   for lc in self.lightcurve]).flatten()
                tmag += moment
            else:
    ...
    def positions_at_epoch(self, epoch):
        ...
        ra = self.ra + elapsed * ra_rate
        return ra, dec

    def make_plot(self, epoch=2018.0, roll=0.0, ra_0=0.0, dec_0=0.0, ax=None):
        ...
        ax.set_title('%s at epoch %s' % (self.name, epoch))
