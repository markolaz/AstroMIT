...
class TestGrid(Catalog):
    ...
    def query(self,
        ...
        if self.randomize_magnitudes:
            self.tmag = np.random.uniform(
                np.min(self.mag_range), np.max(self.mag_range), n)[::-1]
        else:
            self.tmag = np.linspace(
                np.min(self.mag_range), np.max(self.mag_range), n)[::-1]
        ...
        # offset
        self.dec = ((dec_grid - np.mean(dec_grid)) / 3600.0 + dec).flatten()
        self.ra = (ra_grid - np.mean(ra_grid)).flatten() / \
            np.cos(self.dec * np.pi / 180.0) / 3600.0 + ra
        ...
        if self.max_motion > 0:
            self.pmra = np.random.normal(0, self.max_motion, n)
            self.pmdec = np.random.normal(0, self.max_motion, n)
        else:
            self.pmra = np.zeros(n)
            self.pmdec = np.zeros(n)

        # assign temperature for each star
        self.teff = 5800.0 + np.zeros_like(self.ra) # FIXME
