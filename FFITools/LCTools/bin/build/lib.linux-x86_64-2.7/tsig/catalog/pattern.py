                ...
                for j in range(h):
                    if not m['data'][i][j]:
                        ...
                        if ra_lo < r < ra_hi and dec_lo < d < dec_hi:
                            _ra.append(r)
                            _dec.append(d)
        self.ra = np.array(_ra)
        self.dec = np.array(_dec)
        self.pmra = np.zeros(np.size(_ra))
        self.pmdec = np.zeros(np.size(_ra))
        self.tmag = np.zeros(np.size(_ra)) + self.default_tmag
        self.teff = np.zeros(np.size(_ra))
